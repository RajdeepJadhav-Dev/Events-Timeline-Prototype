...
export default function SpaceDayCelebration() {
    const spaceDayEvent = Events.find(event => event.title === 'National Space Day 2024 Celebration');
  ...
  <div className="flex items-center gap-x-4 animate-scroll w-max">
    ...
    {spaceDayEvent?.gallery && Object.values(spaceDayEvent.gallery).map((image, index) => (
      <img
        key={index}
        src={image}
        alt={`Space Day Celebration Image ${index + 1}`}
...
>
  ...
  {spaceDayEvent?.overview && renderOverview(spaceDayEvent.overview)}
</div>
